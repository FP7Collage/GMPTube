package com.Rating
{
	...
	public class Ratings extends UIComponent implements IDataRenderer, IDropInListItemRenderer,	IListItemRenderer
    {
		...
		public function Ratings():void
		{
			...
			//addEventListener(MouseEvent.MOUSE_OVER,handleMouseRoll);
		...
		**/
		...
		private var http_ser:HTTPService=new HTTPService();
		[Bindable] private var star_event:MouseEvent;
		private var star_value:int;
		
		private function handleItemClick(event:MouseEvent):void
		{	
/* 			if ( event.currentTarget.parent.name == 'staticRating')
				return; */
		
		/// setting the busy cursor when click to rate
			CursorManager.setBusyCursor();
			star_event = event;		
						
			var req_param:Object=new Object();
			var t_xml:XML 		= this.document.data;
	/* 		var t_id:String 	= t_xml.id;
			req_param.nodeid 	= t_id;
			req_param.rating 	= Number(event.currentTarget.id); 
			star_value 			= req_param.rating;
			http_ser.request = req_param; */
				
	 
			http_ser.url 	= 	app.ServerPath + app.graphData + "change.php";
			http_ser.method = 	'POST';
			 
			var variables:URLVariables = new URLVariables();
			variables.action  	= 	"changerating";	
			variables.videoid  	= 	t_xml.@id.toString();
			variables.userid	= 	app.Univ_LoginId1;
			variables.rating  	= 	Number(event.currentTarget.id); 

			http_ser.request 	= variables;			
			http_ser.addEventListener(ResultEvent.RESULT,RatingSaved);	
			http_ser.send();	
			...
			app.ratingWindow.visible = false;
		...
		private function RatingSaved(r:ResultEvent):void{
			var rspMessage:String;
			try{
	      		rspMessage = http_ser.lastResult.rsp.message;

           // lines have been removed so that you do not have busy cursor now
		...
		private function gen_pass():void{
			//prevent the default and stop the propogation
			//so that the base class does not select it.
		//	star_event.preventDefault();
		//	star_event.stopImmediatePropagation();
			
			if (star_event.currentTarget.id == 1 && value == 1){
				value=0;
			}
			else{
			//value= star_value;
				value= Number(star_event.currentTarget.id); 
			}
			
			dispatchEvent(star_event);
			CursorManager.removeBusyCursor();	
		}	     
